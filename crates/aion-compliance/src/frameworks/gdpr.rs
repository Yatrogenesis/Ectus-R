...
impl FrameworkImplementation for GDPRFramework {
    ...
    fn assess_compliance(&self, implemented_controls: &[Control]) -> Result<f64> {
        let _total_controls = self.controls.len() as f64;
        let _implemented_count = implemented_controls.len() as f64;
        ...
        for control in &self.controls {
            if !implemented_control_ids.contains(&control.control_id) {
                ...
                gaps.push(ComplianceGap {
                    ...
                    due_date: Some(Utc::now() + chrono::Duration::days(due_days)),
...
impl GDPRAssessment {
    pub fn conduct_dpia(
        ...
        let recommendations = Self::generate_dpia_recommendations(risk_score, &high_risk_factors);
